...
from app.models.models import Parceria, Oferta, Notificacao, Relatorio, StatusParceriaEnum, StatusOfertaEnum
from app.schemas.schemas import ParceriaCreate, ParceriaOut, ParceriaUpdate, NotificacaoOut, RelatorioCreate, RelatorioOut
...
def solicitar_coleta(dados: ParceriaCreate, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
def minhas_parcerias(user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
def atualizar_parceria(parceria_id: int, dados: ParceriaUpdate, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
def listar_notificacoes(user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    return db.query(Notificacao).filter(Notificacao.usuario_id == user_id).order_by(Notificacao.enviada_em.desc()).limit(50).all()


@router_notificacoes.patch("/{notif_id}/lida", response_model=NotificacaoOut)
def marcar_lida(notif_id: int, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
def gerar_relatorio(dados: RelatorioCreate, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    parcerias = (
        db.query(Parceria)
        .join(Oferta)
        .filter(
...
def meus_relatorios(user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    return db.query(Relatorio).filter(Relatorio.gerado_por_id == user_id).order_by(Relatorio.criado_em.desc()).all()

...
from app.models.models import Produto, Oferta, StatusOfertaEnum, Notificacao, TipoNotificacaoEnum
from app.schemas.schemas import ProdutoCreate, ProdutoOut, OfertaCreate, OfertaOut
...
def remover_produto(produto_id: int, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    produto = db.get(Produto, produto_id)
    if not produto or produto.usuario_id != user_id:
...
@router_ofertas.get("/", response_model=List[OfertaOut])
def listar_ofertas(
    ...
    if tipo:
        query = query.filter(Oferta.tipo == tipo)
    if status:
        query = query.filter(Oferta.status == status)
    else:
        query = query.filter(Oferta.status == StatusOfertaEnum.ativa)
    return query.order_by(Oferta.criado_em.desc()).all()


@router_ofertas.get("/{oferta_id}", response_model=OfertaOut)
...
def cancelar_oferta(oferta_id: int, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    oferta = db.get(Oferta, oferta_id)
    if not oferta or oferta.produtor_id != user_id:
        raise HTTPException(status_code=404, detail="Oferta não encontrada ou sem permissão")
